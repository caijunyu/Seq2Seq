...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size, tokenizer=None, normalize_digits=False):
    ...
    if not tf.gfile.Exists(vocabulary_path):
        print('create vocabulary %s from data %s' % (vocabulary_path, ','.join(data_path)))
        vocab = {}
...
def sentence_to_token_ids(sentence, vocabulary,
    ...
    if not normalize_digits:
        return [vocabulary.get(w, UNK_ID) for w in tokens]
    # Normalize digits by 0 before looking words up in the vocabulary.
    return [vocabulary.get(_DIGIT_RE.sub(b"0", w), UNK_ID) for w in tokens]
